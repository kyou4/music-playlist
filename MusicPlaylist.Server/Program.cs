...
app.MapGet(
    "/api/playlists/{id}",
    async (int id, PlaylistDb db) =>
    {
        return
            await db.Playlists.Include(p => p.Tracks).SingleOrDefaultAsync(p => p.Id == id)
...
app.MapPost(
    "/api/playlists",
    async (PlaylistDTO newPlaylist, PlaylistDb db) =>
    {
...
app.MapPut(
    "/api/playlists/{id}",
    async (int id, Playlist updatedPlaylist, PlaylistDb db) =>
    {
...
app.MapGet(
    ...
    {
        return
            await db.Tracks.Include(t => t.Playlist).SingleOrDefaultAsync(t => t.Id == id)
                is Track track
            ? Results.Ok(track)
            : Results.NotFound();
    }
...
app.MapPost(
    "/api/tracks",
    async (Track newTrack, PlaylistDb db) =>
    {
        db.Tracks.Add(newTrack);
        await db.SaveChangesAsync();
        return Results.Created($"/api/tracks/{newTrack.Id}", newTrack);
    }
);

app.MapPut(
    "/api/tracks/{id}",
    async (int id, Track updatedTrack, PlaylistDb db) =>
    {
